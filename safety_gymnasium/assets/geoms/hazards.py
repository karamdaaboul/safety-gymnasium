...
class Hazards(Geom):  # pylint: disable=too-many-instance-attributes
    ...
    color: np.array = field(default_factory=lambda: np.array(COLOR['hazard'], copy=True))
    group: np.array = field(default_factory=lambda: np.array(GROUP['hazard'], copy=True))
